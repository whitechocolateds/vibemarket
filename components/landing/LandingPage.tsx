...
import { LandingPage as Landing, landingVars, storyParagraphs } from '@/lib/landing';
      ...
      <header className={styles.hero}>
        ...
        <div className={`${styles.wrap} ${styles.heroInner}`}>
          ...
          <h1 className={styles.heroTitle}>
            <span className={styles.gradientText}>{landing.heroTitle || product.title}</span>
          </h1>
          {landing.heroLead && <p className={styles.heroLead}>{landing.heroLead}</p>}
        </div>
      </header>
          ...
          <div className={styles.wrap}>
            <figure className={styles.figure}>
              ...
              {landing.problemCaption && (
                <figcaption className={styles.figureCaption}>{landing.problemCaption}</figcaption>
              )}
            </figure>
          </div>
        </section>
          ...
          <div className={styles.wrap}>
            <div className={styles.solution}>
              <div>
                ...
                <h2 className={styles.solutionTitle}>{landing.solutionTitle || product.title}</h2>
                {landing.solutionLead && <p className={styles.solutionLead}>{landing.solutionLead}</p>}
              </div>
              {solutionImage && (
              ...
              {benefits.map((b, i) => {
                ...
                return (
                  <article key={i} className={styles.card}>
                    ...
                    {b.text && <p className={styles.cardText}>{b.text}</p>}
                    {b.check && (
        ...
        <section className={styles.sectionTight}>
          <div className={styles.wrap}>
            <div className={styles.stats}>
              {stats.map((s, i) => (
                <div key={i}>
                  <div className={`${styles.statValue} ${styles.gradientText}`}>{s.value}</div>
        ...
        <div className={`${styles.wrap} ${styles.narrow}`}>
          <div className={styles.cta}>
            <h2 className={styles.ctaTitle}>
              <span className={styles.gradientText}>{landing.ctaTitle || 'Naručite danas'}</span>
            </h2>
